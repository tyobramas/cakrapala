...
export interface ConstellationAnchorData {
  ...
  star4?: { uv: [number, number]; ra: number; dec: number; name: string };
}
...
import { GENERATED_ANCHORS } from "./constellationAnchorsGenerated";

const MANUAL_ANCHOR_OVERRIDES: Record<string, ConstellationAnchorData> = {
  // Kosongkan. Isi hanya kalau ada file .webp yang bukan turunan Stellarium.
};
...
import { buildArtworkMapper, type ArtworkMapper, type AnchorStar } from "./constellationArtworkSolver";

const mapperCache = new Map<string, ArtworkMapper | null>();
const warnedFallback = new Set<string>();

function resolveAnchorKey(abbrOrName: string, profile: ConstellationProfile | null): string | null {
  const cands = [
    profile?.id,
    profile?.abbreviation,
    abbrOrName,
    abbrOrName.slice(0, 3),
  ].filter(Boolean) as string[];
  for (const c of cands) {
    const k = c.toLowerCase().trim().replace(/[^a-z]/g, "");
    if (CONSTELLATION_ANCHORS[k]) return k;
  }
  return null;
}

export function getArtworkMapper(
  abbrOrName: string,
  profile: ConstellationProfile | null
): ArtworkMapper | null {
  const key = resolveAnchorKey(abbrOrName, profile);
  if (!key) {
    const id = profile?.abbreviation || abbrOrName;
    if (!warnedFallback.has(id)) {
      warnedFallback.add(id);
      console.warn(`[ConstellationArt] Tidak ada jangkar untuk "${id}" — pakai fallback quad.`);
    }
    return buildFallbackMapper(profile);
  }
  if (!mapperCache.has(key)) {
    const a = CONSTELLATION_ANCHORS[key];
    const list: AnchorStar[] = [a.star1, a.star2, a.star3, ...(a.star4 ? [a.star4] : [])];
    const m = buildArtworkMapper(list);
    if (m && m.diagnostics.warnings.length && process.env.NODE_ENV !== "production") {
      console.warn(`[ConstellationArt] ${key}:`, m.diagnostics.warnings.join("; "));
    }
    mapperCache.set(key, m);
  }
  return mapperCache.get(key) ?? null;
}

function buildFallbackMapper(profile: ConstellationProfile | null): ArtworkMapper | null {
  if (!profile) return null;
  const ra0 = profile.raHours * 15;
  const dec0 = profile.decDeg;
  const half = (profile.artworkScaleDeg || 30) / 2;
  const cosD = Math.max(0.15, Math.cos((dec0 * Math.PI) / 180));
  return buildArtworkMapper([
    { name: "fb1", uv: [0, 0], ra: ra0 + half / cosD, dec: dec0 - half },
    { name: "fb2", uv: [1, 0], ra: ra0 - half / cosD, dec: dec0 - half },
    { name: "fb3", uv: [1, 1], ra: ra0 - half / cosD, dec: dec0 + half },
  ]);
}
...
export function getConstellationProfile(query: string): ConstellationProfile | null {
  ...
  for (const [key, prof] of Object.entries(CONSTELLATION_PROFILES)) {
    if (
      key === q ||
      prof.id === q ||
      prof.abbreviation.toLowerCase() === q ||
      prof.name.toLowerCase().replace(/[^a-z]/g, "") === q ||
      prof.genitive.toLowerCase().replace(/[^a-z]/g, "") === q
    ) {
      return prof;
    }
  }
  // Cadangan: hanya nama Inggris yang cocok PERSIS, bukan substring.
  for (const prof of Object.values(CONSTELLATION_PROFILES)) {
    if (prof.englishName.toLowerCase().replace(/[^a-z]/g, "") === q) return prof;
  }
  return null;
}
